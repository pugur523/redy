...
TEST(TokenTest, BasicConstruction) {
  ...
  Token tok(TokenKind::kIf, &manager, core::SourceLocation(1, 1, file_id), 2);
  EXPECT_EQ(tok.kind(), TokenKind::kIf);
  EXPECT_EQ(tok.lexeme(), "if");
  ...
  EXPECT_FALSE(tok.dump().empty());
  EXPECT_FALSE(tok.dump_detailed().empty());
  // std::cout << tok.dump() << "\n";
  // std::cout << tok.dump_detailed() << "\n";
}
